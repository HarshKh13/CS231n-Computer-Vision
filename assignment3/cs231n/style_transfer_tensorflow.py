...
def tv_loss(img, tv_weight):
    ...
    img1h = img[:,1:,:,:]
    img2h = img[:,:-1,:,:]
    l1 = tf.reduce_sum((img1h - img2h)**2)
    
    img1w = img[:,:,1:,:]
    img2w = img[:,:,:-1,:]
    l2 = tf.reduce_sum((img1w - img2w)**2)
    
    loss = tv_weight*(l1+l2)
    return loss
    pass
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        ltemp = weight*tf.reduce_sum((gram-target)**2)
        loss = loss + ltemp
...
def gram_matrix(features, normalize=True):
    ...
    features = tf.keras.backend.reshape(features,(-1,C))
...
def content_loss(content_weight, content_current, content_original):
    ...
    """
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    scaler_content_loss = content_weight*(tf.reduce_sum((content_current-content_original)**2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return scaler_content_loss
